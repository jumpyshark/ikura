...
export default function App() {
  ...
  useEffect(() => { Promise.all([loadExpenses(), loadSettings()]).then(([e, s]) => { setExpenses(e); setSettings(s); }).catch(console.error); }, []);
  ...
  const persistExpenses = async (next: Expense[]) => { setExpenses(next); await saveExpenses(next); };
  ...
  const chooseImage = async (camera: boolean) => {
    ...
    const uri = result.assets[0].uri; setImageUri(uri); setProcessing(true);
    try { const recognized = await extractExpense(uri); setDraft(settings.aiFallbackEnabled ? await applyAiFallback(recognized) : recognized); }
    catch (error) { Alert.alert('OCRを実行できません', error instanceof Error ? error.message : '手入力してください。'); setDraft(blank()); }
    finally { setProcessing(false); }
  };
  const submit = async () => {
    if (!draft.storeName.trim() || !/^20\d{2}-\d{2}-\d{2}$/.test(draft.date) || Number(draft.amount) <= 0) return Alert.alert('入力確認', '店舗名、YYYY-MM-DD形式の日付、0円より大きい金額が必要です。');
    ...
    await persistExpenses(next); resetForm(); setMonth(item.date.slice(0, 7)); setTab('history');
  };
  const edit = (item: Expense) => { setDraft(item); setImageUri(item.imageUri); setEditingId(item.id); setTab('add'); };
  const remove = (id: string) => Alert.alert('削除しますか？', 'この支出は元に戻せません。', [{ text: 'キャンセル' }, { text: '削除', style: 'destructive', onPress: () => persistExpenses(expenses.filter((e) => e.id !== id)) }]);
  const updateSettings = async (next: AppSettings) => { setSettings(next); await saveSettings(next); };
  ...
  return <SafeAreaView style={s.safe}><StatusBar style="dark" />
    ...
    <ScrollView contentContainerStyle={s.page} keyboardShouldPersistTaps="handled">
      ...
      {tab === 'add' && <Add draft={draft} setDraft={setDraft} imageUri={imageUri} processing={processing} editing={Boolean(editingId)} categories={settings.categories} chooseImage={chooseImage} submit={submit} reset={resetForm} />}
    ...
    <View style={s.nav}>{([['home','ホーム'],['add','追加'],['history','履歴'],['stats','分析'],['settings','設定']] as [Tab,string][]).map(([key,label]) => <Pressable key={key} style={s.navItem} onPress={() => setTab(key)}><Text style={[s.navText, tab === key && s.navActive]}>{label}</Text></Pressable>)}</View>
  </SafeAreaView>;
}
